  ...
  for lang in args.lang_list.split("_"):
    ...
    if args.dev_path:
      dev_x, dev_y, dev_lang_feat = AppendLangData(
          symbol_table, clusters, args.ngram_order, args.dev_path,
          lang_feat_vector, dev_x, dev_y, lang, dev_lang_feat)
    if args.test_path:
      test_x, test_y, test_lang_feat = AppendLangData(
          symbol_table, clusters, args.ngram_order, args.test_path,
          lang_feat_vector, test_x, test_y, lang, test_lang_feat)

  network = mplm.MNLM(symbol_table.Size(), args.vector_size, args.ngram_order+args.ngram_order-1)#,
                      #dev_lang_feat.shape[1])#remove last parameter for training without lang vectors
