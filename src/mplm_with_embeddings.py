...
class MNLM(object):
  def __init__(self, vocab_size, vector_size, context_size, lang_feat_size=0):
    ...
    self.layers = [layer.Embeddings(vocab_size, vector_size, scale=0.08)]
    if lang_feat_size == 0:
  ...
  def TrainEpoch(self, train_x, train_y, train_lang_feat, batch_size, lr=0.01):
    ...
    for i in range(nbatches):
      ...
      cost = train(train_x[start:end], train_y[start:end], train_lang_feat[start:end])
  ...
  def Test(self, x, y, lang_feat, batch_size=100):
    ...
    for i in range(nbatches):
      ...
      cost = self.test(x[start:end], y[start:end], lang_feat[start:end])
  ...
  def SoftmaxVectors(self, x, y, lang_feat, batch_size=100):
    ...
    for i in range(nbatches):
      ...
      p_matrix = self.softmax_probs(x[start:end], lang_feat[start:end])
  ...
  def ProbVectorGivenPrefix(self, ngram, lang_feat):
    prob_matrix = self.softmax_probs(ngram, lang_feat)
