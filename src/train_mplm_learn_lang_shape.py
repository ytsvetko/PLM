...
parser.add_argument('--brown_clusters', default="/usr1/home/ytsvetko/projects/mnlm/data/pron/brown/pron-text-c5-p1.out/paths")
...
#SEMIVOWELS = set(u"j w y ɥ ʎ jʲ".split())
#VOWELS = set(u"ɐ ʌ ʊ a ɑ e i o u œ æ ɔ ə ɛ ɨ e̯ i̯ o̯ u̯ ɑ̃ ɛ̃ œ̃ ɔ̃ ɑː iː uː ɛː ɔː ʊː ɐː ɪ uːː iːː ɜː".split()) | SEMIVOWELS
#CONSONANTS = set(u"b d f g h i k l m n p r s t v x z ŋ ɲ ʁ ʃ ʒ ʔ ħ sʲ kʲ vʲ mʲ ʲ θ nʲ ʒʲ tʲ xʲ pʲ bʲ ts dz zʲ rʲ ʃʲ dʲ dʒ gʲ tʃ tʃʲ ð lʲ fʲ".split()) | SEMIVOWELS


def LoadData(corpus, symbol_table, clusters, ngram_order, lang):
  ...
  for ngram in next_ngram(corpus):
    x.append([symbol_table.WordIndex(word) for word in ngram[:-1]]+[symbol_table.WordIndex(clusters[word]) for word in ngram[:-1]]+[symbol_table.WordIndex(lang)])
...
def AppendLangData(symbol_table, clusters, ngram_order, corpus_filename, lang_feat_vector_filename, x, y, lang, lang_feat):
  x_lang, y_lang = LoadData(corpus_filename, symbol_table, clusters, ngram_order, lang)
...
def main():
  ...
  for lang in args.lang_list.split("_"):
    ...
    train_x, train_y, train_lang_feat = AppendLangData(
        symbol_table, clusters, args.ngram_order, args.train_path + lang,
        lang_feat_vector, train_x, train_y, lang, train_lang_feat)
    dev_x, dev_y, dev_lang_feat = AppendLangData(
        symbol_table, clusters, args.ngram_order, args.dev_path + lang,
        lang_feat_vector, dev_x, dev_y, lang, dev_lang_feat)

  network = mplm.MNLM(symbol_table.Size(), args.vector_size, args.ngram_order+args.ngram_order-1)#,
